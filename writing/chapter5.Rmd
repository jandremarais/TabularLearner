...
With respect to validation loss, de Brebisson's method performed sightly better, whereas in terms of accuracy, the fixed size of 2 performed the best. 
In datasets where categorical features contain more information, one may however expect this result to be different.
...
In order to test this hypothesis, we implemented a multi-head attention module, as described in [@Song2018].
In this way, our results are comparable to those of produced by a standard neural network.
...
Surprisingly, the model with the attention mechanism performed worse than the MLP on its own; its predictions exhibiting higher variance than the MLP.
...
The SeLU activation function was implemented using its customised weight intitialisation method, but no dropout was used.
In \autoref{fig:selu_all} and \autoref{fig:selu_best} we observe that in terms of validation loss, the model with eight layers using SeLU activations performed the best overall; whereas in terms of accuracy, the model with two layers using ReLU activatios emerged as the winner.
...
![The average performance of ReLU and SeLU activation functions for shallow and deep networks as a function of the number of training epochs.\label{fig:selu_all}](../data/adult/results/activation_depth/all_epochs2.png)
...
Using this mechanism, elementwise addition (or other concatenation functions) may be use in order to combine input and output before and afer a linear layer.
In our empirical work, we investigate the performances of both a shallow and a deep network using skip connections.
The results are summarised in \autoref{fig:resid_epoch} and in \autoref{fig:resid_best}.
On average, the deep neural network performed slightly better in terms of validation loss, whereas... 
Once again, performances are very similar.
...
In this experiment we wanted to investigate the way in which the application of mixup, using various mixup ratios, influences the performance neural networks on the Adult dataset.
...
Therefore we trained a DAE with swap noise for 15 epochs on the Adult dataset.
...
in terms of both accuracy and validation loss, we see that that the pretrained model has an advantage over the classifier that was trained-from-scratch.
...
We do not yet know how to design DAEs in terms of their network architectures, the amount of noise injected, and the lengths of training cycles.
