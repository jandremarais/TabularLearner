...
![Bar plot for each of the categorical features in the Adult dataset.\label{fig:ad_cat}](figures/adult_cat2.png)
...
The lefthandside panel plots the average loss on the validation set over each of the five epochs and the righthandside panel, the average accuracy.
...
We observe that in terms of validation loss, the model with eight layers using SeLU activations performed the best overall; whereas in terms of accuracy, the model with two layers using ReLU activatios emerged as the winner.
...
![Average performance at each epoch for shallow and deep neural networks, with and without skip connections.\label{fig:resid_epoch}](../data/adult/results/residual_depth/all_epochs2.png)
...
In this experiment we wanted to investigate the way in which the application of mixup (own implementation), using various mixup ratios, influences the performance neural networks on the Adult dataset.
...
Un terms of both accuracy and validation loss, we see that that the pretrained model has an advantage over the classifier that was trained-from-scratch.
