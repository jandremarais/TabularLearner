...
$$
\boldsymbol{x}^{\intercal}\boldsymbol{w} + b=y,
$$
...
We may compress the above equation to $\boldsymbol{x}^{\intercal}\boldsymbol{w}=y$, where $\boldsymbol{x}$ includes the bias term and the weight vector $\boldsymbol{w}$ a unit element, *i.e.* $\boldsymbol{x}=[1\quad x_{1}\quad\dots\quad\ x_{p}]^{\intercal}$ is the input, $\boldsymbol{w}=[b\quad w_{1}\quad \dots\quad w_{p}]^{\intercal}$.
...
However the weights are unknown therefore we need to estimate them.
...
Thus we have two different outputs produced by the two neurons, *i.e.*. $z_{1}=\boldsymbol{x}^{\intercal}\boldsymbol{w}_{1}$ and $z_{2}= \boldsymbol{x}^{\intercal}\boldsymbol{w}_{2}$.
In order to produce a final estimate from the initial two estimates, *viz.* $z_{1}$ and $z_{2}$, a third neuron is imported. 
That is, $y=\boldsymbol{z}^{\intercal}\boldsymbol{w}_{3}$, where $\boldsymbol{z}=[z_{1}\quad z_{2}]^{\intercal}$
(See \autoref{fig:simple_nn}).
