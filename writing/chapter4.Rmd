...
The size of the embeddings is a hyperparameter of the model and again there is no way to specify this value *a priori*. 
...
Since a feature may be involved in various different combinations, we use multiple heads to extract multiple combinations, *i.e.* $\{\tilde{\boldsymbol{e}}_{j}^{(h)}\}_{h=1}^{H}$.
In @Song2018 the value of $H$ is set equal to 2, but this is typically a hyperparemeter that needs to be fine-tuned.
After finding the combinatorial features, they are al concatenated into a single vector, *viz.* $\tilde{\boldsymbol{e}}_{j}$.
...
In a vaguely similar idea to attention, in @Zhou2017 the weights to apply to hidden representations are learned automatically.
...
Tabular datasets are usually not as nearly large as unstructured datasets used to train neural networks for image and text classification.
...
Such a of translation can however nog be used in tabular data application, since the inputs are mostly not measured on the same scale and therefore the addition of noise might result in feature values that lie outside the distribution that generated them. 
...
In the MCF approach, noise are generated from some known distribution and added to input values, but this is done manually and cannot be applied to discrete variables.
...
Another input corruption approach that has empirically been shown to work well [here](https://www.kaggle.com/c/porto-seguro-safe-driver-prediction/discussion/44629), is the technique called *swap noise* [@kasar2018].
...
Here artifical samples are create by means of the following formulation:
$$
...
Recall that unsupervised pretraining was described in \label{sec:tl}, and presented as a very effective way of training deep neural networks.
...
That is, we suggest replacing ... with the required classification layers. (please rephrase).
...
Rather using this approach the feature extractor part of the DAE may still be tweaked in order to be optimal for the supervised learning task.
...
In Chapter 3, dropout was discussed as a popular form of regularisation found in many papers [@Song2018], [@Zhang2016], [@Qu2016], [@Guo2017].
...
The exact value for $p$ is typically determined using a grid search, and results in the literature indicate the optimal percentage to be data- and model dependent. (please provide a reference).

Interestingly, @haldar2018 reported that dropout was not effective in their application.
In their analysis of this result, they hypothesise that dropout produces invalid input scenarious which distracts the model.
Therefore the authors rather opt for hand-crafted noise shapes which takes into account the distribution of a relevant feature. 

The other form of regularisation used in the context of tabular data, is weight decay [@Song2018], [@Wang2017b], [@Zhang2016], [@Qu2016] and [@Zhou2017],
...
in tabular data, on the other hand, a very small change in a single feature may result large changes with respect to predictions [@Shavitt2018].
in the latter paper, it is mentioned that this aspect may be addressed by means of including a separate reqularisation term for each of the weights in the network. 
...
This is achieved by minimising the so-called counterfactual loss function, a novel loss function proposed by [@Shavitt2018].
...
The authors made use of the permuation importance algorithm in order to evaluate the importance of features.
...
In order to obtain a more robust estimate and standard errors, we perturbed each feature fives times and measure the NN performance in terms of the log loss function.
We perturbed each feature five times in order to obtain a more robust estimate and to obtain a standard error and we measured the performance in terms of the log loss.
...
To recap the process, we use the predictions of the neural network trained on the Adult dataset as the targer for a boosted model to predict.
...
In terms of hyperparameters specific to a learning algorithm, not much experimentation was done in... ? which paper?
