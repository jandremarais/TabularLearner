...
\usepackage[masters-t, $if(margin-ratio)$$margin-ratio$,$endif$]{usthesis}
$if(beamerarticle)$
...
\usepackage{cleveref} % delete if not working
$if(colorlinks)$
...
\date{$date$}
$if(degree)$
\degree{\AorE{$graad-short$}{$degree-short$}}
       {\AorE{$graad$}
             {$degree$}}
$endif$
$if(faculty)$
\faculty{\AorE{$fakulteit$}
              {$faculty$}}
$endif$
