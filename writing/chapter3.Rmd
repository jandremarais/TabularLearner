...
```{r, fig.cap="A simple single hidden layer autoencoder with 4-dimensional inputs and 2 neurons in the hidden layer.\\label{fig:ae}", out.height="30%"}
...
From observing the types of features extracted for a trained image model (\autoref{fig:zeil}) it becomes clear why they are also effective on other image datasets. The learned filters seemed to look for generic image features like edges and color gradients which are useful for most computer vision tasks.

```{r, fig.cap="Visualising the first layer convolutional filters leared by a neural network in a large image dataset. \\label{fig:zeil}", out.height="50%"}
