...
Through his proposed approaches, the necessity of running complete grid or random searches is eliminated, rendering tremendous improvements in training time and accuracy.
...
A cycle consists of two stages, *viz.* the stage when the learning rate increases to a pre-specified maximum value, and the stage where the learning rate decreases to a chosen minimum value.
...
In 
the first stage the learning rate linearly increases to a certain maximum, and in the second stage the learning rate linearly decreases to a chosen minimum.
As training progresses, many cycles of learning rates occurs.
...
The implication in these setups is that one may use one learning rate cycle together with unusually large learning rates values in order to train neural networks an order of magnitude faster than when using standard training methods.
This phenomenon is called *super-convergence*.
...
As may be expected, the proposed framework focuses on finding the right balance between overfitting and underfitting.
Through consideration of the obtained validation loss during early stages of training, clues are found with respect to more desired values of the hyperparameters.


See \autoref{fig:range_test} for example output of the range test and how to determine the learning rate bounds for the 1Cycle policy.
...
In his most reason work, *viz.* Smith (2018), the author proposes using only a single cycle of CLR, called the *1Cycle* policy.
...
In fact, correct implementation leads to the exploitation of a property called *super-convergence* [@Smith2017], where neural networks can be trained an order of magnitude faster than with standard training methods.
...
It is clear that using the proposed method, the model achieves improved accuracy compared to the standard approach. This is done in an eigth of the number of training iterations required in the original framework.

We now turn to a discussion of how to select hyperparameters in order to achieve super-convergence.
