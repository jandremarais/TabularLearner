...
Still the boom in deep learning research may be said to have started only a bit more than a decode ago, since effective training of a multi-layer network was only made possible after a breakthrough presented in the paper by Hinton *et al.* (2006) (edit the reference).

From that time, tons of contributions have been made to the neural network field: new types of layers, more effective training strategies, and novel approaches to guard against overfitting have been proposed.
A more general definition of *deep learning* encompasses all modern developments in the neural network space.
Because of the large amount of deep learning contribution during the past decade, note that in this chapter we restrict attention to developments that form a basis for deep learning in the context of tabular data.
...
Since representation learning is such an important characteristic of NNs, one group of deep learning developments focus on this aspect. 
...
The autoencoder can be trained in the same way as any other neural network, however the loss function to be minimised is called a  *reconstruction loss*. 
Importantly, note that a reconstruction loss function measures the difference between the reconstructed and actual input, hence the MSE loss function is a common choice in the case of continuous data.
...
It is unsupervised in the sense that it does not require labelling, but it is supervised in the sense that it predicts an ouput.
...
When there are no non-linear activation functions after the linear layers, one can show that this autoencoder is equivalent to application of *Principal Component Analysis* (PCA) to the inputs.
