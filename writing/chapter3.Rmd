...
Instead of limiting the number of neurons in the hidden layer, one may instead use a hidden layer with more neurons than the number of input.
...
```
At this stage, note that data augmentation may be formalised by means of the *Vicinal Risk Minimisation* principle [@Chapelle2001].
...
Note that types of data augmentation that may be applied, depend on the dataset and domain, since all augmentations should preserve the semantic content or the signal in the original observations.
...
A factor which complicates the training of neural networks is the fact that hidden layers have to adapt to the continuously changing distribution of its inputs. 
...
Using ReLUs, careful weight initialisation, and small learning rates should all help a network to deal with internal covariate shifts. 
...
For this purpose @Ioffe2015 proposed *batch normalisation*.
