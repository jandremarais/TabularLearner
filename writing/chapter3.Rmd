...
$$
z_{i}^{(l+1)}=a\left(\boldsymbol{w_{i}}^{(l+1)}\boldsymbol{z}^{(l)}+b_{i}^{(l+1)}\right),
$$
...
z_{i}^{(l+1)}&=a\left(\boldsymbol{w_{i}}^{(l+1)}\tilde{\boldsymbol{z}}^{(l)}+b_{i}^{(l+1)}\right)
\end{aligned}
$$
...
Any new samples with unique feature combinations will likely improve the model's generalisablity.
