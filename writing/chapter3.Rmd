...
In this thesis we use the term *deep learning* to refer to modern developments in the neural network space.
In the last decade, tons of contributions have been made to the field: new types of layers, different ways of training and novel approaches to guard against overfitting have been proposed.
This chapter is devoted to a discussion of the developments that form a basis for a study on deep learning for tabular data.
...
Many deep learning developments focus on this aspect.
An example is the class of neural networks called *autoencoders* (\Sref{sec:ae}).
These neural networks are mostly used as an unsupervised learning method with the aim of aiding us to learn more robust data representations, which may subsequently be transferred to be used by supervised learning algorithms.
The process of transferring knowledge from one network to another is called transfer learning, and is discussed in \Sref{sec:tl}.
...
Since practically all deep learning developments are (either explicitly or implicitly) geared toward preventing an overfit, much of the remainder of this chapter is devoted to proposals that fall in this category.
