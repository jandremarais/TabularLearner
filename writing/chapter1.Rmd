...
DL is receiving a remarkable amount of attention in academia (see \autoref{fig:dlpapers}).
DL has already shown tremendous value in application areas such as *Computer Vision* (CV) [cite], audio processing [cite], and *Natural Language Processing* (NLP) [cite], significantly improving upon the then *State of the Art* (SotA).
In the aformentioned application areas, DL reached a maturity level sufficient to be able to run these systems in a production/commercial environment, *e.g.* voice assistants [cite], face recognition [cite], language translation [cite] to name a few.

![The exponential growth of published papers containing the term *Deep Learning*\label{fig:dlpapers}](figures/dummy.png)

One of the most attractive attributes of DL is its ability to model almost any input-output relationship.
DL has been used to generate art [cite] and music [cite], controlling autonomous cars [cite], playing video games [cite], beating the world's best Go player [cite], suggesting which videos to watch [cite], and improving the quality of images [cite].
...
Although work is being done on the problem [cite] and SotA results were received on rare occasions [cite], the area is nowhere near as mature or receiving as much attention compared to CV and NLP.
ML tasks operating on tabular data are typically more effectively solved by using tree-based methods [cite], which is also evident by looking at the winning solutions of relevant Kaggle competitions[^kaggle].
...
Tabular data is also important. 
