...
def choose_corruption(inp, alpha):
    ...
    for i in range(nrows): 
        col_idx = np.random.choice(ncols, (1,int(ncols*alpha)), replace=False)
        onehot = np.zeros((1,ncols)).astype(np.int64)
        onehot[0, col_idx] = 1
        lambd.append(onehot)
    lambd = np.vstack(lambd)
    return lambd
...
class SwapNoiseCallback(Callback):
    ...
    def on_batch_begin(self, last_input, last_target, train, **kwargs):
        ...
        lambd = [inp.new(choose_corruption(inp, self.alpha)) for inp in last_input]
        shuffle = []
...
class TabularMixUpCallback(Callback):
    ...
    def on_batch_begin(self, last_input, last_target, train, **kwargs):
        ...
        last_cats = torch.cat([emb(last_cats[:,i]) for i,emb in enumerate(self.learner.model.embeds)],1)
...
class AECallback(Callback):
    ...
    def on_batch_begin(self, last_input, last_target, train, **kwargs):
        # last_cats, last_conts = last_input
        # target = torch.cat([emb(last_cats[:,i]) for i,emb in enumerate(self.learner.model.embeds)],1)
        # target = torch.cat([target, last_conts], 1)
