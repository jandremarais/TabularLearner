...
experiment_name, exp_path = create_experiment('embeddingsize_rule', path)

config={'rule':['Wang','Brebisson', 'Constant:2']}
...
for i, params in config_df.iterrows():
    for fold, (train_ids, valid_ids) in enumerate(kf.split(df)):
        ...
        if params['rule'] == 'Constant:2': emb_szs = {var:2 for var in cat_vars}
        elif params['rule'] == 'Brebisson': emb_szs ={}
        elif params['rule'] == 'Wang': 
            emb_szs = {var:int(6*(len(data.train_ds.classes[var])**0.25)) for var in cat_vars}
        ...
        if fold==0: lr = request_lr(learn, wd=1e-5)
